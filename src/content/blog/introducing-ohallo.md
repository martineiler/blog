---
title: "oHallo wants AI to run customer operations from inside your organisation"
description: "A new platform launches with a wager: that AI should work inside a company's own systems, not beside them — turning customer operations from a room full of people doing lookups into something a small team supervises rather than staffs."
date: 2026-06-07
tags: ["ai", "enterprise-ai", "architecture", "platform"]
draft: false
---

In most companies, customer operations is a bridge staffed by people. A customer writes in to ask where an order is, what an invoice covers, whether something can still be returned. Someone reads the message, opens the record in the ERP, checks the policy, types the reply, updates the system. The software that runs the business sits on one side, the customer on the other, and a person spends the day carrying information across the gap. It is slow, it does not scale, and on most support teams it is the larger part of the job.

[oHallo](https://ohallo.ai), launched today out of Copenhagen, is a wager that this bridge should not be staffed by people — and, more pointedly, that the AI doing the carrying should work *inside* a company's own systems rather than beside them. The distinction sounds academic. It is the difference between a tool that changes how customer operations feels and one that changes what it is.

Most attempts at this go wrong in the same way. To answer a customer, software needs to know what an order is, what the return policy says, how pricing works — so the builders model it. They create an order object, a returns workflow, a pricing engine, and import the company's data to feed them. Within a year there is a second system of record sitting next to the real ERP, quietly disagreeing with it: out of sync, locked in to someone else's schema, and unable to answer any question nobody thought to model in advance. The thing the company actually wanted — correct answers drawn from its real systems — is the one thing that architecture makes hardest. oHallo is built on the opposite decision, and it rests on two refusals.
...
What changes, if this works, is not only response times. It is what a customer operations team is for. The bridge that used to absorb most of its day — read the message, look it up, type the reply — runs on its own, inside the systems that already hold the answers. The people move up a layer: they decide how things should be handled, watch the exceptions, and turn the function from a queue that has to be staffed into one that can simply scale. A part of the business that has always been counted as a cost starts to look like something else.

The line on oHallo's site is "the first time it happens you don't quite believe it." It describes a specific moment: a request that would have sat in a queue for half a day resolves itself, correctly, against the company's live systems, while you are still reading the notification. The ambition behind that moment is larger than a faster reply. It is to move customer operations from beside the business to inside it. oHallo is live at [ohallo.ai](https://ohallo.ai).
